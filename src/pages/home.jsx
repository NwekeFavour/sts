...
const HIGHLIGHTS = [
  {
    ...
    desc: "Every child is unique. We build bespoke therapy programmes tailored to each individual's strengths and needs.",
  },
  {
...
const TESTIMONIALS = [
  {
    quote: 'St. Stephens changed our lives. Within months of starting therapy, our son began communicating in ways we never imagined.',
    name: 'Adaeze O.',
    role: 'Parent',
  },
  {
    quote: 'The training programme here gave me both the clinical skills and the empathy to truly serve autistic children and their families.',
    name: 'Emeka T.',
    role: 'Certified Therapist',
  },
  {
    quote: 'From the intake process to the ongoing reports, everything was so clear and compassionate. We finally felt heard.',
    name: 'Ngozi & Femi A.',
    role: 'Parents',
  },
];
      ...
      <section className="relative overflow-hidden pt-26 pb-20 lg:pt-25 lg:pb-32">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-8 z-10">
            <h1 className="text-4xl lg:text-[68px] font-bold tracking-tight text-[#111] leading-[1.1]">
              Every Child <br />
          ...
          <div className="lg:col-span-6 relative flex justify-center lg:justify-end">
            ...
            <div className="relative w-full max-w-[480px] aspect-[4/5] bg-[#E8F8E8] rounded-[40px] rounded-tl-[140px] p-8 flex flex-col justify-between overflow-hidden shadow-sm">
              ...
              <div className="absolute left-4 top-1/3 flex flex-col gap-3 bg-white/80 backdrop-blur-sm p-2 rounded-full shadow-sm z-20">
                {['fb', 'tw', 'ig'].map((s) => (
                  <span key={s} className="w-5 h-5 bg-stone-200 rounded-full block cursor-pointer hover:bg-stone-300 transition-colors" />
                ))}
              </div>
...
/>
              ...
              <div className="absolute bottom-6 left-6 right-6 flex flex-wrap gap-2 justify-center z-10">
                {SERVICES_PREVIEW.map((s) => (
                  <span key={s.name} className="px-4 py-2 bg-white/95 backdrop-blur-xs text-xs font-medium text-stone-800 rounded-full shadow-xs border border-stone-100 transform hover:scale-105 transition-transform duration-200">
                    {s.name}
                  </span>
                ))}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-12 gap-8 items-end mb-16">
            <div className="lg:col-span-7">
              <p className="text-xs font-semibold uppercase tracking-wider text-stone-400 mb-3">Why Choose Us?</p>
            ...
            <div className="lg:col-span-5">
              <p className="text-stone-600 leading-relaxed">
                We blend breakthrough strategies with compassionate care to deliver personalized 
                health solutions that put your child and family first.
              </p>
            </div>
          </div>
            ...
            {HIGHLIGHTS.map((h, i) => (
              ...
              >
                <div className="space-y-4">
                  ...
                  <p className={`text-sm leading-relaxed ${i === 1 ? 'text-stone-800' : 'text-stone-500'}`}>
                    {h.desc}
                  </p>
                </div>
                <div className="pt-6 flex justify-between items-center">
                  <Link to="/services" className="text-xs font-bold uppercase tracking-wider group flex items-center gap-1">
                    Learn More 
                    <span className="transform group-hover:translate-x-1 transition-transform">→</span>
                  </Link>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${i === 1 ? 'bg-black text-white' : 'bg-stone-100 text-stone-600'}`}>
                    →
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 bg-stone-50 rounded-[32px] p-10 border border-stone-100">
            {STATS.map((s) => (
              <div key={s.label} className="text-center space-y-1">
                <p className="text-4xl lg:text-5xl font-bold tracking-tight text-stone-900">{s.value}</p>
                <p className="text-xs font-medium text-stone-400 uppercase tracking-wider">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  <div className="relative max-w-7xl mx-auto px-6">
    ...
    <div className="text-center max-w-2xl mx-auto mb-20 space-y-5">
      ...
      <h2 className="text-4xl lg:text-5xl font-bold tracking-tight text-stone-900 leading-tight">
        Simple Steps to
        <span className="block text-[#513424]">Get the Right Support</span>
      </h2>
          ...
          <div className="bg-stone-900 text-white rounded-[40px] p-8 lg:p-16 grid lg:grid-cols-12 gap-8 items-center relative overflow-hidden">
            <div className="lg:col-span-8 space-y-4 relative z-10">
              ...
              <p className="text-stone-400 text-sm max-w-2xl leading-relaxed">
                Our therapist training programme is built on real clinical practice. Join a cohort of passionate 
                professionals and earn your certification under the mentorship of our expert team.
              </p>
            </div>
            <div className="lg:col-span-4 lg:text-right relative z-10">
              <Link to="/services#training" className="inline-block px-6 py-3.5 bg-[#C5FF4D] text-black font-semibold rounded-full hover:bg-[#b5f03d] transition-colors text-sm">
      ...
      <section className="py-20 bg-stone-50 border-t border-stone-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center max-w-xl mx-auto mb-16 space-y-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-stone-400">Stories</p>
            <h2 className="text-3xl lg:text-4xl font-bold tracking-tight">Words from Families & Therapists</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, idx) => (
              <div key={idx} className="bg-white p-8 rounded-[32px] border border-stone-100 shadow-xs flex flex-col justify-between">
      ...
      <section className="py-24 bg-white text-center">
        <div className="max-w-4xl mx-auto px-6 space-y-6">
          ...
          <p className="text-stone-500 max-w-lg mx-auto text-sm leading-relaxed">
            Whether you're a parent seeking support or a professional looking to grow, we're here for you.
          </p>
          <div className="flex flex-wrap justify-center gap-4 pt-4">
            <Link to="/request" className="px-6 py-3.5 bg-[#513424] text-white font-medium rounded-full hover:bg-[#513424] transition-colors text-sm">
              Request a Therapist
            </Link>
            <Link to="/contact" className="px-6 py-3.5 border border-stone-200 text-stone-700 font-medium rounded-full hover:bg-stone-50 transition-colors text-sm">
              Talk to Us First
            </Link>
          </div>
        </div>
